...
const { exec, createShell } = require("shell");
...
class WebActivity extends ui.Activity {
  ...
  onContentViewSet(contentView) {
    ...
    this.webview.on("console_message", (event, msg) => {
      console.log(
        `${path.basename(msg.sourceId())}:${msg.lineNumber()}: ${msg.message()}`
      );
    });
    ...
    // 事件绑定
    const jsBridge = this.webview.jsBridge;
...
const isProductionMode = myEngine()
  .executionConfig.getProjectConfig()
  ?.buildInfo?.isRelease();
if (isProductionMode) {
...
// ui.SendCode.on("all_activities_destroyed", () => {
//   process.exit();
// });
...
async function runInDevMode() {
  if (!fs.existsSync("./node_modules")) {
    console.info(
      "安装npm依赖中，请保持网络通畅，可能需要几分钟或者更长，若失败请手动在命令行安装"
    );
    await execOrExit("npm i --no-bin-links");
  }
  console.info(
    "开始编译，请耐心等待，若失败可能是之前依赖安装不完全，需要删除node_modules文件夹重新安装依赖"
  );
